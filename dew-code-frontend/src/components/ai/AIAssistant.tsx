...
const AIAssistant: React.FC<AIAssistantProps> = ({ compact, initialMessage, onContextConsumed }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "Hello! I'm your AI coding assistant powered by **Ollama Qwen2.5-Coder 1.5b**.\n\nI can help you:\n• 🐛 Debug and fix errors\n• ⚡ Explain complex code\n• ✨ Optimize performance\n• 🧪 Write tests\n• 📝 Generate boilerplate\n\nPaste your code or use the editor buttons above!",
      timestamp: new Date(),
    },
  ]);
  const [input,  setInput]  = useState('');
  const [typing, setTyping] = useState(false);
  const [model,  setModel]  = useState('qwen2.5-coder:1.5b'); // ✅ fixed: was 'qwen2.5-coder'
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      const aiContent = resp.success && resp.data?.response
        ? resp.data.response
        : resp.message || 'Sorry, I could not get a response.';
  ...
  const clearChat = () => {
    setMessages([{ id: Date.now().toString(), role: 'assistant', content: 'Chat cleared. How can I help you?', timestamp: new Date() }]);
  };
    ...
    <div className="flex flex-col h-full" style={{ background: '#0D0D16' }}>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: '#1A1A26' }}>
        ...
        <div className="ml-auto flex items-center gap-2">
          {/* ✅ Model selector — 1.5b as default and first option */}
          <select
            value={model}
        ...
        <div className="px-3 py-2 border-t overflow-x-auto" style={{ borderColor: '#1A1A26' }}>
          <div className="flex gap-1.5 pb-1" style={{ minWidth: 'max-content' }}>
            {QUICK_ACTIONS.map((action) => (
              <button key={action.label} onClick={() => sendMessage(action.prompt)} disabled={typing}
      ...
      <div className="p-3 border-t" style={{ borderColor: '#1A1A26' }}>
        <div className="flex gap-2 items-end">
          <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)}
            ...
            style={{ background: '#12121A', border: '1px solid #2A2A3A', color: '#E2E8F0', caretColor: '#00D4B8', lineHeight: '1.5' }}
